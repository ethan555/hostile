...
void main()
{
	vec2 p = (resolution.xy - gl_FragCoord.xy) / resolution.y;
	vec3 rd = normalize(vec3(p.xy, 1.064));
	vec4 color = v_vColour * texture2D( gm_BaseTexture, v_vTexcoord );
	// AURORA TIME
    vec4 vcolor = mix(vec4(0.006,0.026,0.095, 1.), vec4(0.007,0.011,0.035, 1.), gl_FragCoord.y / resolution.y);//v_vTexcoord.y);
	vcolor += aurora(rd);
	vcolor = pow(vcolor, vec4(1. / 2.2)); // gamma correction
	vcolor = smoothstep(0., 1.,vcolor);
	vcolor.a = color.a * 1.;
	vcolor.rgb *= color.r;
	vcolor.r = (resolution.y - gl_FragCoord.y) / resolution.y;
    gl_FragColor = vcolor;
}
